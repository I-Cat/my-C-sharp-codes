...
namespace Medical_Store
{
    public class Medicine
    {
     ...
     /*   private int identityNo;
        ...
        public static void UpdateMedicineOnSell(int[] sellQuantity,int[] quantityBeforeSell,int[] id)
        {
            MessageBox.Show("" + quantityBeforeSell.Length);
            ...
            {
                string qry = "update Medicine set Quantity=@quantity where IdentityNo=@oldID ";

                SqlConnection conn = ConnectionManager.GetConnection();
                SqlCommand cmd = new SqlCommand(qry, conn);
                cmd.CommandType = CommandType.Text;
                int ii = 0; int jj = 0;
                int QuantityAfterSell = quantityBeforeSell[ii] - sellQuantity[jj];
                MessageBox.Show("" + QuantityAfterSell);
                cmd.Parameters.AddWithValue("@oldID", item);
                cmd.Parameters.AddWithValue("@quantity", QuantityAfterSell);
                ii++; jj++; k++;
                // return 
                cmd.ExecuteNonQuery();
            }
